...
public class Grammar {
    ...
    public ConvertedGrammar convertToConvertedGrammar() {
        Set<String> nonTerminalSymbols = new HashSet<>(Arrays.asList(nonTerminalVariables));
        Set<String> terminalSymbols = new HashSet<>(Arrays.asList(terminalVariables));

        HashMap<String, ArrayList<String>> convertedProductions = new HashMap<>();
        for (Production production : productions) {
            ArrayList<String> productionList = convertedProductions.getOrDefault(production.getLeftSide(), new ArrayList<>());
            productionList.add(production.getRightSide());
            convertedProductions.put(production.getLeftSide(), productionList);
        }

        return new ConvertedGrammar(nonTerminalSymbols, terminalSymbols, convertedProductions, startingCharacter);
    }
